...
**What replaced it, and it is stronger.** Holding the outage at 40 s and varying only how old the divergence was when the node returned: ~38 s old reconciles in **~1 s**, ~6 s old takes **~31 s**. Disjoint, exact two-sided Mann-Whitney U = 0 at 6v6, **p = 0.0022**. *An older divergence repairs faster.* A third condition (6 s outage) is bimodal at ~0.02 s or ~52 s and is **not explained** by either manipulated variable — recorded as open, with no mechanism named.
